package com.wbillingsley.rubik
...
object Solver {
  ...
  def search(cube:Cube, max:Int, cond:CubeTest, iteration:Int = 0, fromMoves:Seq[Op]):Option[Transform] = {
    // I've put a little printing code at the top that might help
    if (iteration % 100000 == 0) {
      val candidate = Transform(cube, Turn.fromNum(iteration, fromMoves).reverse)
      println(s"Considering candidate $iteration ${candidate.short}")
    }
  ...
  /**
    ...
    */
  def findCrossMove = Solver.searchAndPrint("Cross move", Cube.fresh, ???)
  ...
  /**
    ...
    */
  def findBottomSmToMwMove =  Solver.searchAndPrint("Bottom edge swap", Cube.fresh, ???, Seq(Turn.R, Turn.Ra, Turn.D, Turn.Da))
  ...
  def solveBottomEdgePieces(c:Cube) = for {
    ...
    e <- searchAndPrint("Bottom edge pieces", c, ???, ???)
  } yield e
  ...
  /**
    ...
    */
  val bottomCornersInPlace:CubeTest = ???
  ...
  def placeBottomCorners(c:Cube) = for {
    ...
    e <- searchAndPrint("Position bottom corners", c, ???, ???)
  } yield e
